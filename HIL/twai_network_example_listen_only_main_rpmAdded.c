...
#define ThrottlePotentiometerPin 36
...
TaskHandle_t rpm_reading_task_handle;
...
static void rpm_reading_task(void *arg) {
    uint32_t rpm_value;
    while(1) {
        // Read RPM value from potentiometer
        rpm_value = adc1_get_raw(ADC1_CHANNEL_0); // Assuming potentiometer is connected to ADC channel 0
...
static void throttle_percentage(void *arg) {
    float rpm_factor = 100.0; // Adjust factor based on RPM sensitivity
    ...
    while(1) {
        ...
        // Calculate throttle percentage based on RPM and potentiometer value
        throttle_percentage = ((float)rpm_value / 4095.0) * 100.0;
       
        // Adjust throttle percentage based on RPM
        throttle_percentage += (rpm_value / rpm_factor); // Add RPM effect
 
     
 
        // Ensure throttle percentage is within bounds
        if (throttle_percentage < 0.0) {
            throttle_percentage = 0.0;
        } else if (throttle_percentage > 100.0) {
            throttle_percentage = 100.0;
        }
 
        // Print throttle percentage
        printf("Throttle percentage: %.2f%%\n", throttle_percentage);
...
twai_message_t transmit_message_switch = {.identifier = (0x18530902), .data_length_code = 8, .extd = 1, .data = { 0x03, 0x00, state, 0x00, 0x00, 0x00, 0x00}};
...
xTaskCreate(rpm_reading_task, "rpm_reading_task", 4096, NULL, 5, &rpm_reading_task_handle);
xTaskCreate(throttle_percentage , "throttle_percentage_task", 4096, NULL, 8, NULL);
